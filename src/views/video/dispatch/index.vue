    ...
    <div class="lg:w-8/10 flex flex-col">
      <div class="h-12 bg-slate-50 w-full flex items-center text-xl px-4">
        ...
        <Icon
          ...
          size="30"
          :color="`${state.num === 9 ? '#3b82f6' : ''}`"
          @click="checkGrid(9)"
        />
      </div>
      <div
        :class="`inline-grid ${
          state.num === 4 ? 'grid-cols-2' : state.num === 9 ? 'grid-cols-3' : ''
        }  gap-0.5 w-full h-full`"
      >
        <div
          :class="`bg-black border-2 ${
            state.checkIndex === i ? ' border-pink-700' : ''
          } flex items-center`"
          v-for="i in state.num"
          @click="checkVideo(i)"
        >
          <p
            v-if="isEmpty(state.videoUrl[i - 1])"
            class="w-full text-center font-semibold text-xl m-0 text-gray-50"
            >{{ i }}</p
          >
